...
let t = THREE;
let renderer, camera, width, height, scene, canvas;
...
export default class Tetris {
  ...
  initKeyListener() {
    document.addEventListener('keydown', ({
      ...
      switch (keyCode) {
        case 37:
          this.moveLeftAndRight(0);
          break; // 左移
        case 39:
          this.moveLeftAndRight(1);
          break; // 右移
        case 38:
          this.rotateMatrixView();
          break; // 方向上 旋转
        case 40:
          this.moveDown();
          break; // 方向下
  ...
  rotateMatrixView() {
    ...
    let copy = this.copyArr(this.currentCube.position);
    let type = this.currentCube.type;

    // 某点(ab)绕点（mn）逆时针90度 得点（m+n-b,n-m+a)  
    let cx = Math.round((copy[0][0] + copy[1][0] + copy[2][0] + copy[3][0]) / 4);
    let cy = Math.round((copy[0][1] + copy[1][1] + copy[2][1] + copy[3][1]) / 4);
    //旋转的主要算法. 可以这样分解来理解。  
    ...
    let canRotate = true;
    let minus = this.minusMatrix(copy, pos);
  ...
  generateCube() {
    ...
    let pos = this.currentCube.position;
    let nextPos = this.nextCube.position;
    for (let i = 0; i < pos.length; i++) {
  ...
  initCubes() {
    matrix = [];
    let martixObject = new t.Object3D();
    let cube = new t.BoxGeometry(cubeWidth, cubeWidth, 20);
    let map = new t.TextureLoader().load(getImage());
    map.wrapS = map.mapT = t.RepeatWrapping;
    let material = new t.MeshLambertMaterial({
      map,
      side: t.DoubleSide
    });
    let mesh = new t.Mesh(cube, material);
    let m = 0,
      n = 0;
    // 初始化所有方块设置为隐藏
  ...
  //初始化轮廓线
  initRectLine() {
    let rect = new t.Group();

    let lineMaterial = new t.MeshBasicMaterial({
      color: 0xffffff,
    });
    ...
    let top = new t.Line(TopAnBottomGeometry, lineMaterial);
    ...
    let left = new t.Line(LeftAndRightGeometry, lineMaterial);
    ...
    let start = this.scoreLabel.clone(true);
